 % MODIFIED NEWTON-RAPHSON ALGORITHM 2.3               
 %
...
%   syms('I', 'FP0', 'D','x','s');
 TRUE = 1;
 FALSE = 0;
 fprintf(1,'This is Modified Newtons Method\n');
 ...
 s = input(' ','s');
 FP = inline(s,'x');
 fprintf(1,'Input the 2nd derivative of F(x) in terms of x\n');
 s = input(' ','s');
 FFP = inline(s,'x'); 
 OK = FALSE;
...
% compute P(I)
 FP0 = FP(P0);
 FFP0=FFP(P0);
 D = F0*FP0/(FP0*FP0-F0*FFP0);
% STEP 6
 P0 = P0 - D;  
 F0 = F(P0);
 if FLAG == 2 
 fprintf(OUP,'%3d   %18.12e   %18.12e\n',I,P0,F0);
 end
% STEP 4
 if abs(D) < TOL 
% procedure completed successfully
 fprintf(OUP,'\nApproximate solution = %.12e\n',P0);
 fprintf(OUP,'with F(P) = %.12e\n',F0);
 fprintf(OUP,'Number of iterations = %d\n',I);
 fprintf(OUP,'Tolerance = %.12e\n',TOL);
 OK = FALSE;
% STEP 5
 else
 I = I+1;
