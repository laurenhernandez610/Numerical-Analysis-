...
% STEP 8
 ...
 if FLAGI>=3
   ...
   plot(tt,ya,'r--');
   for I=1:N
    tt=linspace(X(I),X(I+1),21);
    yy=A(I)+B(I)*(tt-X(I))+C(I)*(tt-X(I)).^2+D(I)*(tt-X(I)).^3;
    plot(tt,yy,'b-');
   end;  
  grid on;
  legend('Actual function','Clamped spline')    
 else
  plot(X,A,'ro');hold on;     
  for I=1:N
    tt=linspace(X(I),X(I+1),21);
  ...
  grid on;
  legend('Data points','Clamped  spline')     
 end;    
