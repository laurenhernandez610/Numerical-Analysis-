 ...
 %syms('F','DF','DDF','DDDF','OK','A','B','ALPHA','N','FLAG','NAME','OUP','H');
 %syms('T','W','I','x','s');
 TRUE = 1;
 FALSE = 0;
 fprintf(1,'This is Tayors Method of order four.\n');
 ...
 s = input(' ','s');
 DF = inline(s,'t','y');
 fprintf(1,'Input the function d^2F(t,y)/dt^2 in terms of t and y\n');
 fprintf(1,'For example: y-t^2+1-2*t-2\n');
 s = input(' ','s');
 DDF = inline(s,'t','y');
 fprintf(1,'Input the function d^3F(t,y)/dt^3 in terms of t and y\n');
 fprintf(1,'For example: y-t^2+1-2*t-2\n');
 s = input(' ','s');
 DDDF = inline(s,'t','y');
 ...
 fprintf(OUP, 'Taylor Method of Order 4\n\n');
...
% STEP 1
 ...
 if OKS==TRUE
   ...
   W = ALPHA;
   fprintf(OUP, '    t        w               y-w\n\n'); 
   fprintf(OUP, '%7.5f %15.8e %15.8f\n', T, W, W-W);
 else
   W = ALPHA;
   fprintf(OUP, '    t        w  \n\n');
   fprintf(OUP, '%7.5f %15.8f \n', T, W);
 end;
...
% STEP 3
% Compute W(I)
 W = W+H*(F(T, W)+0.5*H*DF(T,W)+H*H*DDF(T,W)/6.0+H*H*H*DDDF(T,W)/24.0);
 AP(I+1)=W;
...
% STEP 4
 ...
 for i=1:201;
  ...
  if  OKS==TRUE
    plot(xx,AP,'*-'); hold on;
    plot(xxp,yyp,'r-');
    legend('Taylor Method of Order 4','Exact solution', 'Location','NorthWest');
   
  else
    plot(xx,AP,'*-'); hold on;
    legend('Taylor Method of Order 4','Location','NorthWest');
  end; 
