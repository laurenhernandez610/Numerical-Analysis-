...
% OUTPUT:  Approximations W(I) to Y(X(I)) for each I=0,1,...,N+1.
 ...
 if strcmp(reply,'n') %
  ...
  fprintf(1,'Input the exact solution y(x) in terms of x \n');
  fprintf(1,'For example:  -0.1*(sin(x)+3*cos(x))  \n');
  s = input(' ','s');
  SOL = inline(s,'x'); 
 end; % 
 OK = FALSE;
...
% STEP 9 */
 fprintf(OUP, '%3d %13.8f %13.8f %13.8e\n', I, AA, ALPHA, 0.0);
  for I = 1 : N 
  X = AA+I*H;
  yy(I+1)=SOL(X);
  wy(I+1)=W(I);
  fprintf(OUP, '%3d %13.8f %13.8f %13.8e\n', I, X, W(I),abs(SOL(X)-W(I)));
  end;
  I = N+1;
  yy(I+1)=BETA;
  wy(I+1)=BETA;
  fprintf(OUP, '%3d %13.8f %13.8f %13.8e\n', I, BB, BETA, 0.0);
  h1=plot(xx,wy,'*'); hold on;
  h2=plot(xx,yy,'r^-');
%   plot(xx,yy,'r-');
  legend([h1,h2],'Finite Difference Method','Exact solution', 'Location','NorthWest');
  xlabel('x'); ylabel('y'); 
  box on; grid on;   
 else
  I = 0;
